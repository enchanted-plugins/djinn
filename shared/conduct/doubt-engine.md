...
3. You're about to declare a verdict (SHIP, DONE, "looks good", "preservation high enough") on your own output.
...
3. Name one piece of concrete evidence against — a precedent entry,
   a known failure code, an anchor-state fact, a side effect, a missing case.
...
Self-doubt is the only way to catch silent commitments that compound across a long session. A wrong frame in turn 2 becomes scaffolding by turn 8 — and across `/compact`, the scaffolding survives even when the evidence behind it doesn't.
...
| "Just edit the anchor by hand" | "OK, editing." | "Hand-edits to `anchor.json` get overwritten on the next intent-anchor refresh. The honest-numbers contract requires the anchor be captured by D1, not handwritten. Bypass anyway?" |
| *(your own prior claim)* "drift below 0.7 means we're fine" | "Right, no advisory needed." | "I framed that too narrowly — 0.7 is the advisory threshold, not a confidence floor; with N < 5 we should return `insufficient_data`, not silence." |
...
2. Cite the evidence (memory entry, precedent log, anchor fact, file content, observed test).
...
- If it's a recurring pattern across sessions: graduate the precedent entry to auto-memory feedback per `precedent.md` § Relationship to auto-memory.
